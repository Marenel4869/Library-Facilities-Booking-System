...
const basePath = 'C:\\xampp\\htdocs\\Library-Facilities-Booking-System';

// File 1 content
const file1Content = `<?php
...
// Define file paths
const file1Path = path.join(basePath, 'api', 'notify.php');
const file2Path = path.join(basePath, 'notifications.php');
const file3Path = path.join(basePath, 'logs', '.gitkeep');

try {
    // Create directories
    fs.mkdirSync(path.dirname(file1Path), { recursive: true });
    fs.mkdirSync(path.dirname(file3Path), { recursive: true });
    ...
    const lineCount1 = countLines(file1Path);
    const lineCount2 = countLines(file2Path);
    const lineCount3 = countLines(file3Path);

    console.log('✅ Files created successfully!\n');
    console.log('FILE SUMMARY:');
    console.log('='.repeat(70));
    console.log(`FILE 1: ${file1Path}`);
    console.log(`        Lines: ${lineCount1}`);
    console.log();
    console.log(`FILE 2: ${file2Path}`);
    console.log(`        Lines: ${lineCount2}`);
    console.log();
    console.log(`FILE 3: ${file3Path}`);
    console.log(`        Lines: ${lineCount3} (empty file)`);
    console.log();
    console.log('DIRECTORY VERIFICATION:');
    console.log('='.repeat(70));
    console.log(`✓ ${path.dirname(file1Path)} - EXISTS`);
    console.log(`✓ ${path.dirname(file2Path)} - EXISTS`);
    console.log(`✓ ${path.dirname(file3Path)} - EXISTS`);
} catch (error) {
